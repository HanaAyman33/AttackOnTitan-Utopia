...
public class View {
	...
	public void addAllComponents() {
		...
		// Note: weaponShopButton fields removed as they're replaced by weapon cards
	...
	public Button getAntiTitanShellCard() {
		if (Controller.getChoosenMode() != null && Controller.getChoosenMode().equals("Hard")) {
			return HardView.getAntiTitanShellCard();
		} else {
			return EasyView.getAntiTitanShellCard();
		}
	}

	public Button getLongRangeSpearCard() {
		if (Controller.getChoosenMode() != null && Controller.getChoosenMode().equals("Hard")) {
			return HardView.getLongRangeSpearCard();
		} else {
			return EasyView.getLongRangeSpearCard();
		}
	}

	public Button getWallSpreadCannonCard() {
		if (Controller.getChoosenMode() != null && Controller.getChoosenMode().equals("Hard")) {
			return HardView.getWallSpreadCannonCard();
		} else {
			return EasyView.getWallSpreadCannonCard();
		}
	}

	public Button getProximityTrapCard() {
		if (Controller.getChoosenMode() != null && Controller.getChoosenMode().equals("Hard")) {
			return HardView.getProximityTrapCard();
		} else {
			return EasyView.getProximityTrapCard();
		}
	}
	...
	private void updateWallHealthAndDanger(String choosenMode, ArrayList<Lane> l) {
		ArrayList<ProgressBar> wallHealth = "Hard".equals(choosenMode) ? hardView.getWallHealthHard()
				: easyView.getWallHealthEasy();
		ArrayList<Label> dangerLevel = "Hard".equals(choosenMode) ? hardView.getWallDangerLevelHard()
				: easyView.getWallDangerLevelEasy();

		int expectedLanes = "Hard".equals(choosenMode) ? 5 : 3;
		if (wallHealth.size() < expectedLanes || dangerLevel.size() < expectedLanes) {
			System.err.println(
					"Warning: Insufficient UI elements for " + choosenMode + " mode. Expected " + expectedLanes +
	...
	public void performTurnTitans() {
		...
		for (ArrayList<TitanImageView> lane : this.allLanes) {
			for (TitanImageView t : lane) {
				...
				double maxHealth;
				if (t.titan instanceof PureTitan || t.titan instanceof AbnormalTitan)
					maxHealth = 100;
				else if (t.titan instanceof ArmoredTitan)
					maxHealth = 200;
				else
					maxHealth = 1000;
